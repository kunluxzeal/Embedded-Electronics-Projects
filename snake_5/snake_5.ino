...
const int RS = 8;             //Set RS pin as Pin 12
const int Enable = 9;         //Set Enable as Pin 11
const int D4 = 4;              //Set D4 pin as Pin 5
const int D5 = 5;              //Set D5 pin as Pin 4
const int D6 = 6;              //Set D6 pin as Pin 3
const int D7 = 7;             //Set D7 pin as Pin 2
const int Backlight = 10;      //Set the backlight power to pin 9
const int BUTTON = A0;
//Set LCD Size

const int ROWS = 2;                 //2 Rows
const int COLS = 16;                 //16 Columns
...
{ B00000,
  ...
  lcd.print("SNAKEPUZZLE: BY");
 lcd.setCursor(6,2);
  lcd.print("kunluxzeal");
...

   {
     ...
     {  
...

       {
         ...
         if (key==BUTTON_UP && selectedLevel<levels) selectedLevel++; // increase the selectable level

         else if (key==BUTTON_DOWN && selectedLevel>1) selectedLevel--;


   //      else if (key==BUTTON_LEFT && selectedLevels1) selectedLevel--;      
...
// if the  button is pressed, the beginning of the game
...
// if there is a game

  {
   ...
   key = ScanBUTTON(); // check the joystick
...
// select the direction of movement of the snake depending on the position of the joystick
         ...
         if (key==BUTTON_RIGHT && head->dir!=3) head->dir = 2;

         else if (key==BUTTON_UP   && head->dir!=1) head->dir = 0;

         else if (key==BUTTON_DOWN  && head->dir!=0) head->dir = 1;

         else if (key==BUTTON_LEFT && head->dir!=2) head->dir = 3;
...

  {
...

   {
     ...
     {  
       ...
       if (key > 0)// start the game when you move the button in any direction
...

{
  ...
  byte Direction = BUTTON_RELEASED;

  unsigned int Right_button = analogRead(A0);

  unsigned int up_button = analogRead(A0);
  unsigned int down_button = analogRead(A0);

  unsigned int left_button = analogRead(A0);
